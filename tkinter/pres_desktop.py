...
Logo file: place pocki_logo.png in the same folder.
"""
...
# ─────────────────────────────────────────
# LOAD ENV FIRST — must be before create_client
# ─────────────────────────────────────────
load_dotenv()
...
# ─────────────────────────────────────────
# SUPABASE CLIENT — initialized after load_dotenv
# ─────────────────────────────────────────
...
if not _supabase_url or not _supabase_key:
    raise RuntimeError(
        ...
        "Make sure your .env file is in the same folder as pres_desktop.py."
    )
...
class LoginWindow(tk.Toplevel):
    def __init__(self, parent, on_success):
        ...
        # ── Username field uses WHITE bg (not BG) since it's inside the box
    ...
    def _login(self):
        ...
        try:
            ...
            # ── werkzeug hash check (password stored as hash in DB)
